...
case "$1" in
    "start")
        ...
        curl -s http://localhost:80/health || echo "Ошибка подключения, попробуйте через 5 секунд: curl http://localhost:80/health"
        ;;
    ...
    "switch-to-green")
        ...
        echo "Проверка: curl http://localhost:80/health"
        curl -s http://localhost:80/health || echo "Ошибка подключения"
        ;;
